...
if app_mode == "Résumé de Texte":
    ...
    text_to_summarize = st.text_area("Texte à résumer", height=200, placeholder="Entrez votre texte ici...")

    if st.button("Lancer le résumé"):
        if text_to_summarize:
            st.subheader("Résultat")
            if summary_type == "Abstractif (avancé, plus lent)":
                with st.spinner("Génération du résumé abstractif en cours... Veuillez patienter, cela peut prendre un moment."):
                    try:
                        summary = abstractive_summary(text_to_summarize)
                        st.success(summary)
                    except Exception as e:
                        st.error(f"Une erreur est survenue lors du résumé abstractif : {e}")
            else: # Extractif
                with st.spinner("Génération du résumé extractif en cours..."):
                    try:
                        ...
                        st.error(f"Une erreur est survenue lors du résumé extractif : {e}")
        else:
        ...
        if prompt:
            ...
            with st.spinner("Génération du texte en cours..."):
                try:
                    ...
                    st.error(f"Une erreur est survenue lors de la génération du texte : {e}")
        else:
            st.warning("Veuillez entrer un prompt.")

# ----- Pied de page -----
st.sidebar.markdown("---")
st.sidebar.info("Projet DSC559 - Data Science")
